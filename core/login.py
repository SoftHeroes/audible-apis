...
def build_oauth_url(
    ...
    # return httpx.URL(base_url, params=oauth_params),code_verifier
    oauth_params = {
...
class DjangoAudibleLogin:
    ...
    def build_start_url(self):
        ...
        # url,code_verifier = build_oauth_url(
        #     country_code=self._marketplace.country_code,
        #     domain=self._marketplace.domain,
        #     market_place_id=self._marketplace.market_place_id,
        #     client_id=build_client_id(serial=self._serial),
    ...
    def request(self, method, url, **kwargs):
        ...
        while response.status_code == 302:
            ...
            redirect_url = self._last_response.headers['Location']
            ...
            if resource_path != '/ap/maplanding':
                ...
                if redirect_url.find(base_url) == -1:
                    redirect_url = base_url + redirect_url
                
                request = httpx.Request('GET', redirect_url, headers=headers)
        ...
        #         # response = requests.get(redirect_url,headers={"User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/80.0.3987.149 Safari/537.36",})
                
        #         # response = self._session.request( method='GET', url=redirect_url)
                
        #         self._last_response = response
        #         self._last_request = response.request
        #         self._last_response_content = self._last_response.content
                
        #         # fp = urllib.request.urlopen(redirect_url)
        #         # my_bytes = fp.read()
        #         # my_str = my_bytes.decode("utf8")
                
        #         # self._last_response = fp
        #         # self._last_response.content = my_bytes
        #         # self._last_response.url = httpx.URL(redirect_url)
        #         # self._last_response.text = my_str
        #         # self._last_response.status_code = 200
